...
def main():
	...
	if network == 'Covariance':
		...
		print("Running automatic. Log file: %s_CovCommunity.out" %  treeSet)
	...
	if network == 'Affinity':
		...
		# Run automatic plateau finder
		print("Running automatic. Log file: %s_AffAuto.out" %  (plateau, treeSet))
		os.system("%s -trees -f %s -ft Trees -w 0 -r %s -o Community -t Affinity -cm %s -lm auto -dm URF -am Exp" % (clvPath, treeSet, rooted, model, plateau)+\
